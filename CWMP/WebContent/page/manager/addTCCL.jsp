	...
	<body class = "top" style="background: url(/CWMP/img/1.jpg);">
		...
		<div class = "total" style="margin:0% 5% 0% 5%; height: 680px;" >
			...
			<div class = "operation">
				...
				当前位置 : 管理员&gt;授课管理&gt;添加<br/><br/><br/>
				<center>
					<form action="/CWMP/mAddTCCLServlet" method="post">
				...
				班级：
						<select id="select2" name="select2" onchange="selectChange2()">
						<%
							CLassDao cDao = new CLassDao();
							List<CLass> listCLass = cDao.QueryAll();
							for(int i=0; i<listCLass.size(); i++){
								String CLno = listCLass.get(i).getCLno();
						%>
								<option><%=CLno %></option>
						<%	
							}
				...
				课程：
						<select id="select3" name="select3" onchange="selectChange3()">
						<%
							CourseDao c = new CourseDao();
							List<Course> listCourse = c.QueryAll();
							for(int i=0; i<listCourse.size(); i++){
								String Cname = listCourse.get(i).getCname();
						%>
								<option><%=Cname %></option>
						<%	
							}
