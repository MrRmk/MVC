				...
				<div class = "showTable">
					...
					<center>
						<table class = "tableSeting">
					  	...
					  	<%	
					  		List<TCCL> list = (List<TCCL>)request.getAttribute("listTCCL");
					  		System.out.println("***授课表添加数据前********************");
					  		//获取设置的list值，在jsp页面间传递值用
					  		if( list!=null ){
						  		for(int i=0; i<list.size(); i++){
						  		System.out.println("***授课表添加了"+(i+1)+"条数据********************");
								...
								String Cname = ct.QueryByCno(Cno).getCname().toString();
						  		System.out.println("mTCCL.jsp页面"+Tno+Tname+CLno+Cname);
						%>
					  			<tr>
					  				<td><%=i+1 %></td>
					  				<%-- <td><%=Tno %></td> --%>
					  				...
					  				<%-- <td><%=Cno %></td> --%>
					  				<td><%=Cname %></td>
					  				<%-- <td><a href="/CWMP/page/manager/updateTCCL.jsp?Tno=<%=Tno%>&Tname=<%=Tname%>&CLno=<%=CLno%>&Cno=<%=Cno%>&Cname=<%=Cname%>">修改</a></td> --%>
									<td><a href="mDeleteTCCLServlet?Tno=<%=Tno%>&Tname=<%=Tname%>&CLno=<%=CLno%>&Cno=<%=Cno%>&Cname=<%=Cname%>">删除</a></td>
					  			</tr>
